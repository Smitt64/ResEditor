...
public:
    ...
    enum
    {
        ...
        TypeItemRole,
        GroupRole,
        DragIconRole // иконка для плашки перетаскивания (если задана —
                     // берется вместо Qt::DecorationRole)
    };
    explicit ToolBoxModel(QObject *parent = nullptr);
    virtual ~ToolBoxModel();

    virtual QVariant data(const QModelIndex &index, int role) const Q_DECL_OVERRIDE;
    // Поддерживается только DragIconRole — иконка плашки перетаскивания
    // (иконка самого элемента в списке не меняется)
